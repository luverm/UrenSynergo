...
const PERIOD_KEY = "synergo-period";
const ENTRIES_KEY = "synergo-entries";
...
export default function UrenOverzicht() {
  ...
  useEffect(() => {
    try { localStorage.setItem(ENTRIES_KEY, JSON.stringify(entries)); } catch (e) { console.error(e); }
  }, [entries]);

  const totalHours = entries.reduce((s, e) => s + e.hours, 0);
  const maxHours = entries.length > 0 ? Math.max(...entries.map((e) => e.hours)) : 1;

  const resetForm = () => { setTask(""); setHours(""); setNote(""); setSelectedIcon(0); setSelectedColor(0); setEditIdx(null); setShowForm(false); };

  const handleSubmit = () => {
    if (!task.trim() || !hours) return;
    const entry = { task: task.trim(), hours: parseFloat(hours), note: note.trim(), icon: ICONS[selectedIcon], color: COLORS[selectedColor] };
    if (editIdx !== null) { const u = [...entries]; u[editIdx] = entry; setEntries(u); }
  ...
  const handleEdit = (i) => {
    const e = entries[i];
    setTask(e.task); setHours(String(e.hours)); setNote(e.note || "");
    setSelectedIcon(Math.max(0, ICONS.indexOf(e.icon))); setSelectedColor(Math.max(0, COLORS.indexOf(e.color)));
    setEditIdx(i); setShowForm(true);
  };

  const handleDelete = (i) => setEntries(entries.filter((_, idx) => idx !== i));
      ...
      <div style={{ maxWidth: 520, margin: "0 auto" }}>
        ...
        {showForm && (
          <div style={{ padding: 20, borderRadius: 16, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", marginBottom: 20, animation: "slideDown 0.4s cubic-bezier(.22,1,.36,1) both", overflow: "hidden" }}>
            ...
            <button onClick={handleSubmit} disabled={!task.trim() || !hours} style={{
              width: "100%", padding: "14px", borderRadius: 12, border: "none",
              background: task.trim() && hours ? "linear-gradient(135deg, #2CE874, #1BA854)" : "rgba(255,255,255,0.06)",
              color: task.trim() && hours ? "#0E1117" : "rgba(255,255,255,0.2)",
              fontSize: 15, fontWeight: 700, cursor: task.trim() && hours ? "pointer" : "default",
              fontFamily: "'DM Sans', sans-serif", transition: "all 0.25s",
            }}>
              {editIdx !== null ? "✓ Bijwerken" : "✓ Toevoegen"}
            </button>
          </div>
        )}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, marginLeft: 12 }}>
                        ...
                        <button onClick={() => handleEdit(i)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.25)", cursor: "pointer", fontSize: 14, padding: 4 }}>✏️</button>
                        <button onClick={() => handleDelete(i)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.25)", cursor: "pointer", fontSize: 14, padding: 4 }}>🗑️</button>
                      </div>
                    </div>
                    <Bar hours={item.hours} max={maxHours} color={item.color} />
                  </div>
                </div>
              ))}
            </div>
